...
byte correctionLaunch(void)
{
  ...
  {
    launchValue = (100 + configPage6.lnchFuelAdd);
  }
      ...
      if ((currentStatus.coolant > (int)(configPage6.egoTemp - CALIBRATION_TEMPERATURE_OFFSET)) && (currentStatus.RPM > (unsigned int)(configPage6.egoRPM * 100)) && (currentStatus.TPS <= configPage6.egoTPSMax) && (currentStatus.O2 < configPage6.ego_max) && (currentStatus.O2 > configPage6.ego_min) && (currentStatus.runSecs > configPage6.ego_sdelay) && (BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) == 0) && (currentStatus.MAP <= (configPage9.egoMAPMax * 2)) && (currentStatus.MAP >= (configPage9.egoMAPMin * 2)))
      {
        ...
        if (configPage6.egoAlgorithm == EGO_ALGORITHM_SIMPLE)
        {
          ...
          if (currentStatus.O2 > currentStatus.afrTarget)
          {
            ...
            } // Means we're at the maximum adjustment amount, so simply return that again
          }
          ...
          {
            AFRValue = currentStatus.egoCorrection;
          } // Means we're already right on target
        }
        ...
        {
          AFRValue = 100;
        } // Occurs if the egoAlgorithm is set to 0 (No Correction)
      } // Multi variable check
      else
      {
        AFRValue = 100;
      } // If multivariable check fails disable correction
...
int8_t correctionCrankingFixedTiming(int8_t advance)
{
  ...
  if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
  {
    ...
    } // Use the CLT compensated cranking ignition angle
  }
...
int8_t correctionIdleAdvance(int8_t advance)
{
  ...
  if ((configPage2.idleAdvEnabled >= 1) && (runSecsX10 >= (configPage2.idleAdvDelay * 5)) && idleAdvActive)
  {
    ...
    if ((currentStatus.RPM < (configPage2.idleAdvRPM * 100)) && ((configPage2.vssMode == 0) || (currentStatus.vss < configPage2.idleAdvVss)) && (((configPage2.idleAdvAlgorithm == 0) && (currentStatus.TPS < configPage2.idleAdvTPS)) || ((configPage2.idleAdvAlgorithm == 1) && (currentStatus.CTPSActive == 1)))) // closed throttle position sensor (CTPS) based idle state
    {
  ...
  */
  ...
  {
    idleAdvActive = false;
  } // Clear flag if engine isn't running anymore
  ...
  if (configPage6.engineProtectType == PROTECT_CUT_IGN || configPage6.engineProtectType == PROTECT_CUT_BOTH)
  {
    if (currentStatus.RPMdiv100 >= configPage4.SoftRevLim) // Softcut RPM limit
    {
      ...
      if (softLimitTime < configPage4.SoftLimMax)
      {
        ...
        } // delay timing by configured number of degrees in relative mode
        else if (configPage2.SoftLimitMode == SOFT_LIMIT_FIXED)
        {
          ignSoftRevValue = configPage4.SoftLimRetard;
        } // delay timing to configured number of degrees in fixed mode
...
// Quickshifter adjustments for flat shift KYLE
int8_t correctionSoftFlatShift(int8_t advance)
{
  static uint32_t flatSStartTime = 0;
  static uint32_t lastFlatSRetardTime = 0;
  ...
  // Check if we're allowed to trigger again (500ms debounce)
  bool debouncePassed = (now - lastFlatSRetardTime) > 1000000UL;

  // Check flat shift condition
  ...
  // Trigger only if condition is met and debounce time has passed
  if (flatSCondition && debouncePassed)
  {
    flatSStartTime = now;
    lastFlatSRetardTime = now;
    flatSActive = true;
  }

  // Remain active for 150ms after trigger
  if (flatSActive && ((now - flatSStartTime) <= 300000UL))
  {
    BIT_SET(currentStatus.status5, BIT_STATUS5_FLATSS);
    ignSoftFlatValue = configPage6.flatSRetard;
  }
...
uint16_t correctionsDwell(uint16_t dwell)
{
  ...
  {
    tempDwell = div100(dwell) * currentStatus.dwellCorrection;
  }
